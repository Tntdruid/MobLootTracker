...
-- MobLootTrackerItemTooltip.lua – FINAL CLEAN VERSION
...
---------------------------------------------------------
-- MAIN ITEM TOOLTIP HANDLER (runs ONCE)
---------------------------------------------------------
local function MLT_HandleItemTooltip(tooltip, link)
    if not link then return end

    -- Prevent double-calls
    if tooltip.MLT_AlreadyProcessed then return end
    tooltip.MLT_AlreadyProcessed = true
    ...
    if #loot > 0 then
        ...
        for _, mob in ipairs(loot) do
            local rate = mob.kills > 0 and (mob.count / mob.kills * 100) or 0
            tooltip:AddLine(string.format("%s (%.1f%%, %d drops)", mob.name, rate, mob.count))
        end
    end
    ...
    if #skin > 0 then
        ...
        tooltip:AddLine("|cff88ff88Skinned from:|r")
        for _, mob in ipairs(skin) do
            tooltip:AddLine(string.format("%s (%d skins)", mob.name, mob.count))
        end
    end
end

---------------------------------------------------------
-- RESET FLAG BEFORE EACH TOOLTIP
---------------------------------------------------------
GameTooltip:HookScript("OnTooltipCleared", function(self)
    self.MLT_AlreadyProcessed = nil
end)
